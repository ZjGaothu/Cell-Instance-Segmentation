...
def get_args():
    ...
    parser.add_argument("--img_dir", help="data directory", default="/home/gaozijing/gzj/PRseg/data/dataset1/testuint8/", type=str)
    parser.add_argument("--modelname", help="model name", default="u2net", type=str)
    parser.add_argument('--scale', type=int, default=320, help='resize shape')
    parser.add_argument("--themodel", help="model name", default="u2net_bce_itr_106000_train_0.253333_tar_0.008061", type=str)
...
#     net.load_state_dict({k.replace('module.',''):v for k,v in torch.load(model_dir).items()})
    ...
    for i_test, data_test in enumerate(test_salobj_dataloader):
        ...
        # 放缩至原图大小
        imo = im.resize((628,628),resample=Image.BILINEAR)
        pb_np = np.array(imo)
        # 获取文件名
        aaa = img_name.split(".")
        bbb = aaa[0:-1]
